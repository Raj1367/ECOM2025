...
const CardList = ({ title }: { title: string }) => {
  ...
  return (
    <div className="bg-primary-foreground p-4 rounded-lg border shadow-xs">
      <h1 className="text-md font-medium mb-6">{title}</h1>
      <div className="flex flex-col gap-2 h-[500px] overflow-y-auto p-2">
        {list.map((item) => (
          <Card key={item.id} className="flex-row items-center justify-between gap-4 p-4 bg-secondary">
